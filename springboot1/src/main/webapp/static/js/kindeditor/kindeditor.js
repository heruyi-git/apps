...
// alt
function _alt(el, key, fn) {
	var self = this;
	key = /^\d{2,}$/.test(key) ? key : key.toUpperCase().charCodeAt(0);
	_bind(el, 'keydown', function(e) {
		if (e.altKey && e.which == key && !e.shiftKey && !e.ctrlKey) {
			fn.call(el);
			e.stop();
		}
	});
			...
			if(ret.error=="0")
				if(document.all){
					window.clipboardData.setData("Text",url);
				     alert("远程地址复制成功，请切换换到远程图片界面使用Ctrl+v复制到图片地址栏！"); 
				}else{
					prompt("图片上传成功，请手动复制一下地址到远程地址栏",url);
				}
			else
				alert(ret.message);
		};
...
function _create(expr, options) {
	...
	// update by hry
	if("undefined" == typeof knode[0].style){
		options.srcElement = expr;
		if (!options.width) {
			options.width = expr.style.width;
		}
		if (!options.height) {
			options.height = expr.style.height;
		}
	}else{
		if (!options.width) {
